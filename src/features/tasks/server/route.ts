...
const app = new Hono()
  ...
  .patch(
    ...
    async (c) => {
      const user = c.get('user');
      const databases = c.get('databases');
      const { name, status, projectId, dueDate, description, assigneeId } =
        c.req.valid('json');
      const { taskId } = c.req.param();
  ...
  .post(
    ...
    async (c) => {
      ...
      if (!workspaceId) return c.json({ error: 'workspace is required' });
      ...
      if (!member) return c.json({ error: 'Unauthorized' });
